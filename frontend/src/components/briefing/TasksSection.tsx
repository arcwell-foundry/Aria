    ...
    <div className="flex items-center gap-3 p-3 bg-slate-700/30 border border-slate-600/30 rounded-lg">
      <div
        className={`flex-shrink-0 p-2 rounded-lg ${
          isOverdue ? "bg-critical/10" : "bg-slate-600/50"
        }`}
      >
        {isOverdue ? (
      ...
      <div className="flex-1 min-w-0">
        ...
        {task.due_date && (
          <p className={`text-xs ${isOverdue ? "text-critical" : "text-slate-400"}`}>
  ...
  return (
    ...
    >
      <div className="space-y-4">
        {overdue.length > 0 && (
          <div>
            <h4 className="text-xs font-medium text-critical uppercase tracking-wider mb-2">
              Overdue
            </h4>
