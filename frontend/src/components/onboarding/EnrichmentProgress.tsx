  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2 text-center">
        {isComplete ? (
          <h2 className="font-display text-[24px] leading-[1.3] text-[#1A1D27] aria-settle">
            ARIA now knows {qualityScore ?? 0}% of what she needs about{" "}
            {companyName}
          </h2>
        ) : (
          <h2 className="font-display text-[24px] leading-[1.3] text-[#1A1D27]">
      ...
      {!isComplete && (
        <p className="font-sans text-[13px] leading-relaxed text-[#6B7280] text-center italic">
...
function PresenceMark({ isComplete }: { isComplete: boolean }) {
  if (isComplete) {
    return (
      <div className="relative w-20 h-20 aria-settle">
        ...
        <div className="absolute inset-0 rounded-full bg-[#5A7D60]/10" />
        <div className="absolute inset-2 rounded-full bg-[#5A7D60]/8" />
        <div className="absolute inset-4 rounded-full bg-[#5A7D60]/6" />
      </div>
    );
  }
  ...
  return (
    <div className="relative w-24 h-24">
      ...
      <div
        className="absolute inset-0 rounded-full bg-[#5B6E8A]/10 aria-glow"
        aria-hidden="true"
      />
      {/* Middle breathing layer */}
      <div
        className="absolute inset-3 rounded-full bg-[#7B8EAA]/12 aria-breathe"
        style={{ animationDelay: "1s" }}
        aria-hidden="true"
      />
      {/* Inner drift layer — organic movement */}
      <div
        className="absolute inset-6 rounded-full bg-[#5B6E8A]/15 aria-drift"
        aria-hidden="true"
      />
      {/* Core — subtle steady presence */}
      <div
        className="absolute inset-8 rounded-full bg-[#5B6E8A]/20 aria-breathe"
...
function QualityIndicator({ score }: { score: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <div className="w-full max-w-[200px] h-1 rounded-full bg-[#E2E0DC] overflow-hidden">
        <div
          className="h-full rounded-full bg-[#5A7D60] transition-all duration-[800ms] ease-out"
