...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'goal_execution_plans' AND policyname = 'goal_execution_plans_select'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'goal_execution_plans' AND policyname = 'goal_execution_plans_insert'
    ) THEN
...
DO $$ BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'goal_execution_plans' AND policyname = 'goal_execution_plans_update'
    ) THEN
